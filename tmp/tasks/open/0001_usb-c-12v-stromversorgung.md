...
- [x] **B2 — `PW_EN` unnötig global (optional, kein Defekt).** *(erledigt 2026-09-24: in lokale Labels `+5V` umbenannt, zusätzlich `VBUS` vor F1 benannt; Netzliste bis auf die zwei Netznamen identisch, ERC 0 Fehler)* Korrigiert 2026-09-06: `PW_EN` (Enable MP2359) und `/Layout/EN` (Chip-Enable/Reset ESP32: R9, C7, S2, J6, U6 Pin 3) sind **verschiedene Signale**. Beide `PW_EN`-Labels liegen auf demselben Blatt, das Netz ist geschlossen, ERC meldet nichts. Eine Zusammenlegung wäre schädlich — S2 würde die eigene Versorgung abschalten.
